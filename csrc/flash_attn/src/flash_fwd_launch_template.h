...
#if __CUDACC_VER_MAJOR__ >= 11 && __CUDACC_VER_MINOR__ >= 7
...
#define KERNEL_PARAM_MODIFIER
 ...
 DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_splitkv_kernel_aws, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, int MaxPages) {
     #if defined(ARCH_SUPPORTS_FLASH)
         flash::compute_attn_splitkv_aws<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, MaxPages>(params);
     #else
         FLASH_UNSUPPORTED_ARCH
     #endif
 }
 
 DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_splitkv_combine_kernel_aws, int kBlockM, int Log_max_splits, bool Is_even_K, int MaxPages) {
     static_assert(Log_max_splits >= 1);
     flash::combine_attn_seqk_parallel_aws<Kernel_traits, kBlockM, Log_max_splits, Is_even_K, MaxPages>(params);
 }
 ...
 template<typename Kernel_traits, bool Is_causal>
 void run_flash_splitkv_fwd_aws(Flash_fwd_params &params, cudaStream_t stream) {
     ...
     const int num_m_block = (params.seqlen_q + Kernel_traits::kBlockM - 1) / Kernel_traits::kBlockM;
     dim3 grid(num_m_block, params.num_splits > 1 ? params.num_splits : params.b, params.num_splits > 1 ? params.b * params.h : params.h);
     const bool is_even_MN = params.cu_seqlens_q == nullptr && params.cu_seqlens_k == nullptr && params.seqlen_k % Kernel_traits::kBlockN == 0 && params.seqlen_q % Kernel_traits::kBlockM == 0;   // seqlen_q 为 1 时，is_even_MN 为 false
     const bool is_even_K = params.d == Kernel_traits::kHeadDim;
     BOOL_SWITCH(is_even_MN, IsEvenMNConst, [&] {
         EVENK_SWITCH(is_even_K, IsEvenKConst, [&] {
             LOCAL_SWITCH((params.window_size_left >= 0 || params.window_size_right >= 0) && !Is_causal, Is_local, [&] {
                     ...
                     BOOL_SWITCH(params.knew_ptr != nullptr, Append_KV, [&] {
                         ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                             SOFTCAP_SWITCH(params.softcap > 0.0, Is_softcap, [&] {
                                 // MAXPAGES_SWITCH(params.max_num_pages_per_seq, [&] {
                                 constexpr int MaxPages = 32;
                                 auto kernel = &flash_fwd_splitkv_kernel_aws<Kernel_traits, Is_causal, Is_local && !Is_causal, Has_alibi, IsEvenMNConst && !Append_KV && IsEvenKConst && !Is_local && Kernel_traits::kHeadDim <= 128, IsEvenKConst, Is_softcap, Split, Append_KV, MaxPages>;
                                 if (smem_size >= 48 * 1024) {
                                     C10_CUDA_CHECK(cudaFuncSetAttribute(
                                         kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
                                 }
     ...
     if (params.num_splits > 1) {
         ...
         EVENK_SWITCH(is_even_K, IsEvenKConst, [&] {
             ...
             constexpr int MaxPages = 32;
             if (params.num_splits <= 2) {
                 flash_fwd_splitkv_combine_kernel_aws<Kernel_traits, kBlockM, 1, IsEvenKConst, MaxPages><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
             } else if (params.num_splits <= 4) {
                 flash_fwd_splitkv_combine_kernel_aws<Kernel_traits, kBlockM, 2, IsEvenKConst, MaxPages><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
             } else if (params.num_splits <= 8) {
                 flash_fwd_splitkv_combine_kernel_aws<Kernel_traits, kBlockM, 3, IsEvenKConst, MaxPages><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
             } else if (params.num_splits <= 16) {
                 flash_fwd_splitkv_combine_kernel_aws<Kernel_traits, kBlockM, 4, IsEvenKConst, MaxPages><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
             } else if (params.num_splits <= 32) {
                 flash_fwd_splitkv_combine_kernel_aws<Kernel_traits, kBlockM, 5, IsEvenKConst, MaxPages><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
             } else if (params.num_splits <= 64) {
                 flash_fwd_splitkv_combine_kernel_aws<Kernel_traits, kBlockM, 6, IsEvenKConst, MaxPages><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
             } else if (params.num_splits <= 128) {
                 flash_fwd_splitkv_combine_kernel_aws<Kernel_traits, kBlockM, 7, IsEvenKConst, MaxPages><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
             }
             C10_CUDA_KERNEL_LAUNCH_CHECK();
             // });
         });
 ...
 template<typename T, int Headdim, bool Is_causal>
 void run_mha_fwd_splitkv_dispatch_aws(Flash_fwd_params &params, cudaStream_t stream) {
     constexpr static int kBlockM = 64;  // Fixed for all head dimensions
     constexpr static int kBlockN = Headdim <= 64 ? 256 : (Headdim <= 128 ? 128 : 64);
     run_flash_splitkv_fwd_aws<Flash_fwd_kernel_traits<Headdim, kBlockM, kBlockN, 4, false, false, T>, Is_causal>(params, stream);
 }
